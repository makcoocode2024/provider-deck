...
import { activeOption, confidenceLabel, reasoningUiState, selectionFor, stateMessage } from "../domain/reasoning";
...
export function SettingsPage() {
  ...
  return <div className="page-content narrow"><div className="page-heading"><div><h1>设置</h1><p>控制网络、安全和配置写入策略。</p></div><button className="button primary" disabled={saveState === "saving"} onClick={save}>{saveState === "saving" ? <LoaderCircle className="spin" size={17} /> : saveState === "saved" ? <Check size={17} /> : <Save size={17} />}{saveState === "saving" ? "保存中" : "保存设置"}</button></div>
    ...
    <section className="settings-section"><h2>能力未探明时的回退档位</h2><p className="section-note">每个模型的真实推理档位由该模型自己的能力探测决定，在“编辑服务 → 确认模型”里选择。这里只管一件事：某个模型的推理能力还没探明时，配置写出该用哪个旧档位兜底。</p><SettingSwitch label="自动推荐回退档位" description="按当前服务的模型参数量、上下文窗口和本机可用显存推算，保存时重新计算。" checked={draft.autoReasoningMode} onCheckedChange={toggle("autoReasoningMode")} />{!draft.autoReasoningMode && <label>手动回退档位<select aria-label="手动回退档位" value={draft.manualReasoningLevel} onChange={(event) => setDraft({ ...draft, manualReasoningLevel: event.target.value as ReasoningLevel })}>{legacyReasoningLevels.map((level) => <option value={level} key={level}>{level}</option>)}</select></label>}<div className="setting-row"><span><Gauge size={17} /><span><strong>当前回退档位：{draft.effectiveReasoningLevel}</strong><small>{draft.reasoningMatchMessage ?? "仅在模型推理能力未探明时使用；已探明的模型按各自能力写出。"}</small></span></span></div></section>
 ...
 */
function ReasoningSummary({ provider }: { provider: Provider }) {
